...
for i =1:length(all_model)
    ...
    fig, clb = get_subplot_T2(model, pChip_wat, H, P, "Li", scale, Cliq_max,  Li_content, dpi; ylabel = true, label_on = label_on)
    push!(plots, fig)
    # push!(clbs, clb)

    fig, clb = get_subplot_T2(model, pChip_wat, H, P, "extraction", scale, Extract_epi, Li_content, dpi; ylabel = false, discr = true)
    push!(plots, fig)
    # push!(clbs, clb)    
...
annotate!(plt, 6.5, 20.0, text("Li enrichment [C/C0]",         :black, :center, 15, "bold", "Arial"), subplot=1)
annotate!(plt, 6.5, 20.0, text("Extraction event [#]",         :black, :center, 15, "bold", "Arial"), subplot=2)
annotate!(plt, 6.5, 20.0, text("Extraction temperature [°C]",  :black, :center, 15, "bold", "Arial"), subplot=3)

savefig(plt, "Figure_MM_KM_BA_T_v2.png")
